...
namespace ProjectEternity.Core.Online
{
    ...
    public class Master
    {
        ...
        {
            ServerManagerInfo RedirectServer = ListServerManager[Random.Next(ListServerManager.Count)];
            string[] ArrayFullIP = RedirectServer.IP.Split(new string[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
            if (ArrayFullIP[0] == "127.0.0.1")
            {
                if (NewClient.IP.StartsWith("192.168"))
                {
                    ArrayFullIP[0] = "192.168.2.12";
                }
                else if (!NewClient.IP.StartsWith("127.0.0.1"))
                {
                    ArrayFullIP[0] = "70.51.202.227";
                }
            }

            NewClient.Send(new RedirectScriptServer(ArrayFullIP[0], int.Parse(ArrayFullIP[1]), "", 0));
            NewClient.Close();
        }
